...
from sqlalchemy import update, bindparam, select, and_, delete, null, or_
...
async def get_shop_info_by_name(db: AsyncSession, shop_name_list: List[str]):
...
async def get_shop_product_list_for_cost_table(
    ...
    return {
        "shopInfos": await get_shop_info_by_name(db, list(shop_name)),
        "dbData": db_data,
...
async def update_shop_product_card_table(
    db: AsyncSession,
