...
async def product_detail_scraper(mock_sub_scraper: PwKreamPageSubScraper):
    await mock_sub_scraper.page_handler.go_to(product_detail_path)
    yield mock_sub_scraper


@pytest.fixture(scope="module")
async def buy_and_sell_scraper(mock_sub_scraper: PwKreamPageSubScraper):
    await mock_sub_scraper.page_handler.go_to(buy_and_sell_path)
    yield mock_sub_scraper


@pytest.fixture(scope="module")
async def trading_volume_scraper(mock_sub_scraper: PwKreamPageSubScraper):
    await mock_sub_scraper.page_handler.go_to(trading_volume_path)
    yield mock_sub_scraper
...
def test_trading_volume_extract_target_date(
    ...
    # When
    trading_volume_scraper._set_target_date(50)

    # Then
    assert trading_volume_scraper.target_date == date.today() - timedelta(days=50)
...
async def test_trading_volume_filter_trading_volume_data(
    ...
    # Given
    trading_volume_scraper._set_target_date(1000)
