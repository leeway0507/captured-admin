import os
import pytest
from shop_scrap.shop_list.consortium import PwConsortiumListSubScraper
from playwright.async_api import async_playwright
from components.browser_handler import PwPageHandler

html_path = __file__.rsplit("/", 1)[0]
...
async def sub_scraper():
    ...
    sub_scraper.late_binding(pw_page, "consoritum")
    sub_scraper.allocate_job("adidas")
    await sub_scraper.page_handler.go_to(test_html)
    yield sub_scraper
...
async def test_extradct_info(sub_scraper: PwConsortiumListSubScraper, list_result):
    ...
    assert preprocessed_data == test_data
