...
async def sub_scraper_mock():
    ...
    sub_scraper.late_binding(pw_page, "harresoe")
...
@pytest.mark.anyio
async def test_extract(sub_scraper_mock: PwHarresoeListSubScraper):
    # When
    # await sub_scraper.page_handler.go_to(html("seven_store"))
    raw_html = await sub_scraper_mock._extract()

    assert len(raw_html) == 624


@pytest.mark.anyio
async def test_extract_info(sub_scraper_mock: PwHarresoeListSubScraper):
    # When
...
@pytest.mark.anyio
async def test_price(sub_scraper_mock: PwHarresoeListSubScraper):
    card_list = await sub_scraper_mock.extract_card_html()
