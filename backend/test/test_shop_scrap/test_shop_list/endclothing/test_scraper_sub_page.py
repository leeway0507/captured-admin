import os
import pytest
from shop_scrap.shop_list.endclothing import PwEndClothingPageSubScraper
...
async def sub_scraper():
    ...
    sub_scraper = PwEndClothingPageSubScraper()
...
@pytest.mark.anyio
async def test_get_card_info(sub_scraper: PwEndClothingPageSubScraper):
...
@pytest.mark.anyio
async def test_get_size_info(sub_scraper: PwEndClothingPageSubScraper):
    # When
    size_info = await sub_scraper.get_size()
    print(size_info)

    # Then
    assert size_info[0] == {"shop_product_size": "UK 8.5", "kor_product_size": "UK 8.5"}
