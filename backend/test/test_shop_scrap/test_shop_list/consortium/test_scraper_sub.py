import os
import pytest
from shop_scrap.shop_list.consortium import PwConsortiumPageSubScraper
...
def html(name: str):
    return f"file://{os.path.join(curr_path,'html',name+'.html')}"
...
@pytest.mark.anyio
async def test_seven_store_get_card_info(sub_scraper: PwConsortiumPageSubScraper):
    # When
    # await sub_scraper.page_handler.go_to(html("seven_store"))
...
@pytest.mark.anyio
async def test_consortium_scrap_size(sub_scraper: PwConsortiumPageSubScraper):
    await sub_scraper.page_handler.go_to(html("consortium"))
    # When
...
@pytest.mark.anyio
async def test_consortium_get_size_info(sub_scraper: PwConsortiumPageSubScraper):
    # When
...
@pytest.mark.anyio
async def test_consortium_scrap_product_id(sub_scraper: PwConsortiumPageSubScraper):
    # When
...
@pytest.mark.anyio
async def test_consortium_get_card_info(sub_scraper: PwConsortiumPageSubScraper):
    # When
