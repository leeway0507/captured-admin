...
def optimize_image(root_dir: str, file_name: str):
    ...
    if img.width < 1000:
        img.save(resize_path, quality=80, optimize=True, format="WEBP")

    target_width = 1000
    target_height = round(target_width / aspect_ratio)
    resized_img = img.resize((target_width, target_height))
    resized_img.save(resize_path, quality=80, optimize=True, format="WEBP")
...
def create_thumbnail(root_dir: str, file_name: str):
    ...
    cropped_imag_white_bg = Image.new(
        "RGBA", cropped_img.size, "WHITE"
    )  # Create a white rgba background
    cropped_imag_white_bg.paste(cropped_img, (0, 0), cropped_img)
    ...
    centered_image.paste(cropped_imag_white_bg, paste_position)
