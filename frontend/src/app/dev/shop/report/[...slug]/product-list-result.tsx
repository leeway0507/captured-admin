...
import { insertShopListData } from "../../fetch-report";
...
interface fetchDataProps {
    shop_name: string;
    scrap_time: string;
    job: { brand_name: string; status: string }[];
    brand_list: string[];
    db_update: boolean;
    num_process: number;
}
...
export default function ProductListResult({ fetchData }: { fetchData: fetchDataProps }) {
    ...
    const scrapResult = job.reduce((acc: { [key: string]: string }, cur: any) => {
        acc[cur.job] = cur.status;
        return acc;
    }, {});
    ...
    useEffect(() => {
        ...
        router.push(`/dev/kream/report/list/${reStartScrapName}`);
    ...
    return (
        <div className="flex flex-col gap-4 max-w-3xl mx-auto py-8">
            ...
            <div className="py-4 pb-12">
                ...
                {Object.entries(scrapResult).map(([brandName, jobStatus], idx) => (
            ...
            <div className="flex gap-4 w-full text-xl">
                ...
                <button
                    ref={submitRef}
                    onClick={() => handleSubmit(configState, setReStartScrapName, submitRef)}
                ...
                {/* <button
                    ...
                    onClick={() => handleDelete(`${scrapTime}`)}>
                    {" "}
                    수집 결과 삭제
                </button> */}
                <Link href="/dev/shop/report/all" className="black-bar p-4 text-xl">
                    돌아가기
                </Link>
            </div>
        </div>
...
const handleDB = async (scrapName: string) => {
    await insertShopListData(scrapName).then((res) => {
        if (res.status === 200) {
