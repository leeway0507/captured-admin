...
import { insertShopPageData } from "../../fetch-report";
...
export default function ProductPageResult({ fetchData }: { fetchData: fetchDataProps }) {
    ...
    useEffect(() => {
        ...
        router.push(`/dev/shop/report/page/${reStartScrapName}`);
    }, [reStartScrapName, router]);

    const handleDB = async (scrapDate: string) => {
        await insertShopPageData(scrapDate).then((res) => {
            console.log(res);
            if (res.status === 200) {
    ...
    return (
        <div className="flex flex-col gap-4 max-w-3xl mx-auto py-8 ">
            ...
            <div className="flex gap-4 w-full text-lg">
                ...
                <button
                    ref={submitRef}
                    onClick={() => handleScrapProductPage(configState, setReStartScrapName, submitRef)}
